...
const particles = [];
const START_COUNT = 40;  // Startpartikel
const MAX_COUNT = 220;   // Maximal im Strudel
...
function drawParticles() {
  ...
  for (let p of particles) {
    ...
    if (phase === 'float') {
      ...
      // Strudel-/Wirbeleffekt
      const dx = p.x - center.x;
      const dy = p.y - center.y;
      const angle = Math.atan2(dy, dx);
      const swirl = 0.2;      // Drehung
      const speed = rand(1.5, 3); // Geschwindigkeit
      p.x += Math.cos(angle + swirl) * speed + rand(-0.2, 0.2);
      p.y += Math.sin(angle + swirl) * speed + rand(-0.2, 0.2);
      p.alpha -= 0.003;

      if (p.alpha <= 0) {
        // Zur Mitte zurücksetzen
        p.x = center.x + rand(-10, 10);
        p.y = center.y + rand(-10, 10);
        p.alpha = rand(0.6, 1);
        p.radius = rand(1.5, 3);
      }
    }
  ...
  // Optionaler subtiler Kreis-Effekt um Lichtkugel
  if (phase === 'explode') {
    ctx.beginPath();
    ctx.arc(center.x, center.y, 120 + Math.sin(timer * 0.03) * 40, 0, Math.PI * 2);
...
// Neue Partikel aktivieren (nach Explosion)
