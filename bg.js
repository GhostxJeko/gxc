...
// bg.js – Partikel-Tunnel mit starkem Strudel (neutral -> bunt)
...
const particles = [];
const particleCount = 120; // Anfangs weniger Partikel für klare Sicht
...
function createParticles() {
    ...
    for (let i = 0; i < particleCount; i++) {
        // Anfangs neutral: Weiß-Blau
        const hues = [210, 240]; // Weiß-Blau neutral
...
function drawParticles() {
    ...
    for (let p of particles) {
        ...
        if (phase === 'float') {
            ...
            // Starker Spiral-Strudel
            const dx = p.x - center.x;
            const dy = p.y - center.y;
            const distance = Math.sqrt(dx*dx + dy*dy);
            const angle = Math.atan2(dy, dx);
            const swirl = 0.3; // viel stärkere Spiralrotation
