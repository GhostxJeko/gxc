...
const particles = [];
const TOTAL_PARTICLES = 200; // maximale Partikelanzahl
...
function createParticles() {
  ...
  for (let i = 0; i < TOTAL_PARTICLES; i++) {
    const hue = rand(210, 240); // Weiß-Blau Töne
    particles.push({
      x: rand(0, width),
      y: rand(0, height),
      vx: rand(-0.5, 0.5),
...
function drawParticles() {
  ...
  for (let p of particles) {
    ...
    const force = Math.min(0.05, 1/(dist*0.02 + 0.1));
    p.vx += dx * force;
    p.vy += dy * force;

    // Strudelbewegung
    const angle = Math.atan2(dy, dx);
    const swirl = Math.sin(timer * 0.02) * 0.15; // stärkerer Strudel
    p.vx += Math.cos(angle + Math.PI/2) * swirl;
    p.vy += Math.sin(angle + Math.PI/2) * swirl;

    // Explosion-Effekt nach Lichtkugel
    if (timer % 800 > 400) { // Zyklisch: 400 Frames sammeln → 400 Frames explodieren
      const explosionSpeed = 2 + Math.sin(timer*0.01)*1;
  ...
  // Kosmischer Strudel-Effekt
  ctx.beginPath();
  ctx.arc(center.x, center.y, 120 + Math.sin(timer*0.03)*50, 0, Math.PI*2);
