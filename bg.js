// bg.js – Ultimative Partikel-Universum Animation (weiß/blau, Lichtkugel & Strudel)
...
// Partikel erzeugen – zufällig im ganzen Screen
...
function drawParticles() {
    ...
    for (let p of particles) {
        ...
        // Float-Phase: Partikel schweben frei
        if (phase === 'float') {
        ...
        // Gather-Phase: Alle Partikel ziehen zur Mitte → Lichtkugel
        ...
        // Explode-Phase: Strudelartige Explosion
            ...
            // Reset Partikel nach Ausblenden
            if (p.alpha <= 0) {
                p.x = random(0, width);
                p.y = random(0, height);
...
function animate() {
    ...
    // Phasensteuerung: float → gather → explode → float
    if (timer % 2000 === 0) phase = 'gather';
    if (timer % 2000 === 1000) phase = 'explode';
    if (timer % 2000 === 1700) phase = 'float';
}
