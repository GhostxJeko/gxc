// bg.js – Partikel-Tunnel mit kugelförmigem Start & starker Strudel
...
const particles = [];
const particleCount = 120; // Anfang: weniger Partikel
...
function createParticles() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const radius = random(0, Math.min(width, height)/4); // kreisförmig
    const hues = [210, 240]; // Blau & Weiß
...
function drawParticles() {
  ...
  for (let p of particles) {
    ...
    if (phase === 'float') {
      ...
      const swirl = 0.2; // kräftige Spiralbewegung
      ...
      if (p.alpha <= 0) {
        ...
        const explosionHues = [0, 45, 210, 240, 60];
